...
fn main() {
	...
	let mut spinner = Animation::from_file("spinner", &atlas).fail();
	...
	gui.Slider(ID!("roughness")).pip_pos = 0.286;
	...
	for i in (0..100).chain((0..100).step_by(1).rev()).map(|i| f32(i) / 100.).cycle() {
...

		{
			...
			let m = magnification;
			let model = &translate(rotate(scale(identity(), v3(m, m, m)), 90_f32.to_radians(), -V3::x_axis()), v3(0., 0., -0.3));
