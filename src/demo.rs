...
impl DeferredMesh {
	fn draw(&mut self, name: &str) -> bool {
		...
		if task.is_none() {
			...
			*task = Some(task::Runtime().spawn({
				let name = name.to_string();
				async move { (name.clone(), Model::new_cached(&name)) }
			}));
		}

		if task.as_ref().valid().is_ready() {
			let (n, m) = task::Runtime().finish(mem::take(task).valid());
